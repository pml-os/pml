  ...
  for (i = 0; i < 4; i++)
      ...
      for (j = 0; j < 16; j++)
	{
	  ...
	  if (i == 0)
	    ...
	    {
	      s0 = rorl (w[(j + 1) & 0xf], 7) ^
		rorl (w[(j + 1) & 0xf], 18) ^ (w[(j + 1) & 0xf] >> 3);
	      s1 = rorl (w[(j + 14) & 0xf], 17) ^
		rorl (w[(j + 14) & 0xf], 19) ^ (w[(j + 14) & 0xf] >> 10);
	      w[j] += s0 + w[(j + 9) & 0xf] + s1;
	    }
	  s1 = rorl (ah[4], 6) ^ rorl (ah[4], 11) ^ rorl (ah[4], 25);
	  ch = (ah[4] & ah[5]) ^ (~ah[4] & ah[6]);

	  t1 = ah[7] + s1 + ch + sha256_k[(i << 4) | j] + w[j];
	  s0 = rorl (ah[0], 2) ^ rorl (ah[0], 13) ^ rorl (ah[0], 22);
